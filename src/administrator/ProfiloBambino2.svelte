<script lang="ts">
  ...
  onMount(async () => {
    try {
            ...
            const res = await axios({method:'get',url:'http://127.0.0.1:8000/singolo_bambino',headers:{'CUSTOM-OPTION':IDn,'Authorization':'Token ' + sessionStorage.getItem('key')}}).then(response=>{
...
<svelte:head>
    ...
    <script src="../specific_page_js/profilo_bambino2.js"></script>

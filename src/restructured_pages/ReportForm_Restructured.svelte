...
import { identity } from 'svelte/internal';
  ...
  onMount(async () => {
    ...
    try {
        axios.all([
            axios({method:'get',url:'http://127.0.0.1:8000/singolo_bambino',headers:{'CUSTOM-OPTION':IDn,'Authorization': 'Token ' + sessionStorage.getItem("key")}}).catch(function(err){
                error404_1=true;
            }),
            axios({method:'get',url:'http://127.0.0.1:8000/report_giornaliero',headers:{'CUSTOM-OPTION':IDn,'Authorization': 'Token ' + sessionStorage.getItem("key")}}).catch(function(err){
                error404_2=true;
            })
...
<svelte:head>
    ...
    <script src="../specific_page_js/report_form.js"></script>
</svelte:head>
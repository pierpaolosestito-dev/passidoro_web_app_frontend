...
$(document).on('click', '#editButton', function(){ 
  ...
  $(document).on('click', '#confirmButton', function(){ 
    ...
    email_padre = $("#email-padre").text();
    email_madre = $("#email-madre").text();
    alert(email_padre);
    if($('#emailPadre').val() != ""){
    ...
    Swal.fire({
      ...
      didOpen: function(){
        aggiornamenti_bambino = {"ID":IDn,"Email_Genitore1":email_padre,"Email_Genitore2":email_madre,"Orario_uscita":orario_uscita};
        if(avatarModificato){
          aggiornamenti_bambino = {"ID":IDn,"Email_Genitore1":email_padre,"Email_Genitore2":email_madre,"Avatar":avatar,"Orario_uscita":orario_uscita};
        }
        axios({method:"put",url:"http://127.0.0.1:8000/singolo_bambino",data:aggiornamenti_bambino,headers:{"Authorization":"Token " +sessionStorage.getItem("key")}})
    .then(response => {

...
$(document).on('click', '#registra-btn', function(){ 
  ...
  if($('#nome-bambino').val().length == 0 || $('#cognome-bambino').val().length == 0 || $('#data-bambino').val() == null || $('#orario-bambino').val() == null|| $('#sezione').val() == null || $('#email-padre').val().length == 0 && $('#email-madre').val().length == 0 )  {
    console.log($("input[name='gender']:checked").val());
   
    if($('#nome-bambino').val().length == 0  ||  $('#cognome-bambino').val().length == 0 || $('#data-bambino').val() == null || $('#orario-bambino').val() == null|| $('#sezione').val() == null || ($("input[name='gender']:checked").val() != "maschio" && $("input[name='gender']:checked").val() != "femmina")  ){
    ...
    bambino = {'Nome':nome_bambino,'Cognome':cognome_bambino,'Email_Genitore1':email_padre,'Email_Genitore2':email_madre,'Data_di_nascita':data_bambino,'Orario_uscita':orario_bambino,'Avatar':avatar,'NomeSezione':sezione};
    console.log("ORARIO_BAMBINO",orario_bambino);
    Swal.fire({
      title:"Caricamento in corso",
      imageUrl:"../media_resources/loader_resources/cat_loader.gif",
      didOpen: function(){
        axios({method:"post",url:'http://127.0.0.1:8000/singolo_bambino',data:bambino,headers:{"Authorization":"Token " + sessionStorage.getItem("key")}})
        .then(response => {
...
$(document).on('click', '#formatimg', function(){ 
  $('#default-avatar').attr("src", "https://t3.ftcdn.net/jpg/01/28/56/34/360_F_128563455_bGrVZnfDCL0PxH1sU33NpOhGcCc1M7qo.jpg");
  $('#registra-btn').prop("disabled", false);
  avatar = "default-avatar";
  $('#formatimg').fadeOut();
});
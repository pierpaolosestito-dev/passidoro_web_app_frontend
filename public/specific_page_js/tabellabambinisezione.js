$(document).on('click', '#excel-btn', function(){
    axios({method:"get",url:" http://127.0.0.1:8000/bambini",headers:{"CUSTOM-OPTION":$('#tabellabambini').attr('name'),"Authorization":"Token " + sessionStorage.getItem("key")}}).then(response=>{
        console.log(response.data);
        arrayofjsonmodified = []
        for(let i=0;i<response.data.length;i++){
            object = {"NOME":response.data[i]['Nome'],"COGNOME":response.data[i]['Cognome'],"EMAIL_GENITORE1":response.data[i]['Email_Genitore1'],"EMAIL_GENITORE2":response.data[i]['Email_Genitore2'],"DATA_DI_NASCITA":response.data[i]['Data_di_nascita'],"NOME_SEZIONE":response.data[i]['NomeSezione']}
                        ...
                        didOpen: function(){
                            axios({
                                ...
                                if(response.data == "Password verificata"){
                                    axios({
                                        method:"delete",
                                        url:"http://127.0.0.1:8000/bambini/"+$('#tabellabambini').attr('name').toLowerCase(),
                                        headers:{"Authorization": "Token " + sessionStorage.getItem("key")}
                                    }).then(response=>{
